...
10. Required fields have editor validation, arrays have sensible minimums and maximums, and reference inputs target only the intended document type. Route slugs are generated from titles and required, with document-type-scoped uniqueness validation. Singular route lookups return `null` when no unique document matches, so duplicate imported content is never selected arbitrarily. Do not store derived module or lesson numbers; the data layer derives them from array order.
   If malformed imported data places one lesson more than once in the same course, the data layer leaves that lesson's module context empty rather than selecting an arbitrary position. Static route slug helpers also deduplicate imported slug values.
...
Course and lesson data returned by the data layer may contain computed `moduleNumber` and `lessonNumber` values for rendering, but those values must be derived from array indexes and never queried from or stored in Sanity. A missing or duplicate slug returns `null` for singular lookups and an empty array for collections. A lesson with multiple owning courses requires the optional course slug to resolve context; without it, the data layer leaves course context empty rather than guessing.
